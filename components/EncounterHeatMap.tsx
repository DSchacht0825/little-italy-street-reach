...
export default function EncounterHeatMap({ locations }: EncounterHeatMapProps) {
  ...
  useEffect(() => {
    ...
    // Initialize map
    try {
      // Default center to Little Italy, San Diego
      const defaultCenter: [number, number] = [-117.1683, 32.7228]
      ...
      map.current.on('load', async () => {
        ...
        // Load and add Little Italy neighborhood boundary
        try {
          const response = await fetch('/little-italy-boundary.geojson')
          const littleItalyBoundary = await response.json()

          // Add Little Italy boundary source
          map.current?.addSource('little-italy-boundary', {
            type: 'geojson',
            data: littleItalyBoundary,
          })

          // Add Little Italy boundary outline layer (bottom layer)
          map.current?.addLayer({
            id: 'little-italy-boundary-fill',
            type: 'fill',
            source: 'little-italy-boundary',
            paint: {
              'fill-color': '#3b82f6',
              'fill-opacity': 0.05,
            },
          ...
          // Add Little Italy boundary outline
          map.current?.addLayer({
            id: 'little-italy-boundary-line',
            type: 'line',
            source: 'little-italy-boundary',
            paint: {
              'line-color': '#1e40af',
          ...
          console.error('Failed to load Little Italy boundary:', error)
        }
    ...
    <div className={isFullscreen ? 'fixed inset-0 z-50 bg-white' : 'relative'}>
      ...
      {isFullscreen && (
        <div className="absolute top-4 left-4 z-10 bg-white px-3 py-2 rounded-lg shadow-lg">
          ...
          <p className="text-xs text-gray-500">City of Little Italy boundary shown in blue</p>
        </div>
      )}
      ...
      {!isFullscreen && (
        <p className="text-sm text-gray-600 mt-3">
          ...
          on map. City of Little Italy boundary shown in blue. Click clusters to expand, click individual points for details.
        </p>
      )}
    </div>
  )
